...
class DoctorHome extends Component {
  ...
  componentDidMount() {
    ...
    fetch(`http://127.0.0.1:3001/doctor/getDoctorMetrics/${doctor.DOC_ID}`)
      .then((res) => res.json())
      .then((res) => {
        if (res !== "NO")
          this.setState({
    ...
    if (this.state.docDetails) {
      fetch(
        `http://127.0.0.1:3001/doctor/getAppointments/${this.state.docDetails.DOC_ID}`
      )
        .then((res) => res.json())
        .then((Appointments) => {
  ...
  handleConfirmApp = (a, i) => {
    ...
    // console.log(load);
    fetch("http://127.0.0.1:3001/doctor/confirmAppointment", load)
      .then((res) => res.json())
      .then((res) => {
        console.log(res);
        if (res === "Confirmed") {
  ...
  handleCancelApp = (pat, i) => {
    ...
    fetch(
      `http://127.0.0.1:3001/doctor/appointments/cancelApp?doc_id=${d_id}&pat_id=${p_id}&date=${dat}&time_slot=${time}`,
      load
    )
      .then((res) => res.json())
      .then((res) => {
  ...
  getConfirmedApp = (heading) => {
    ...
    if (this.state.confApp.length) {
      ...
      fetch("http://127.0.0.1:3001/doctor/getConfAppointments/" + d_id)
        .then((res) => res.json())
        .then((res) => {
  ...
  handleTreated = (b, i) => {
    ...
    fetch("http://127.0.0.1:3001/doctor/treated", load)
      .then((res) => res.json())
      .then((res) => {
  ...
  handleDinCome = (a, i) => {
    ...
    let url = `http://127.0.0.1:3001/doctor/missedAppointment?doc_id=${a.DOC_ID}`;
  ...
  getHistory = (heading) => {
    ...
    if (this.state.history.length) {
      ...
      fetch("http://127.0.0.1:3001/doctor/getHistory/" + d_id)
        .then((res) => res.json())
        .then((res) => {
          if (res === "NO" && this._isMounted) {
            this.setState({
  ...
  getFeedBack = (heading) => {
    ...
    if (this.state.feedBacks.length) {
      ...
      fetch("http://127.0.0.1:3001/doctor/getFeedbacks/" + d_id)
        .then((res) => res.json())
        .then((res) => {
          if (res === "NO" && this._isMounted) {
            this.setState({
