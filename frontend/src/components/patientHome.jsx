...
class PatientHome extends Component {
  ...
  handleGetFeedback(){
    ...
    fetch(`http://127.0.0.1:3001/patient/getFeedback/${patient.PATIENT_ID}`)
    .then((res) => res.json())
    .then((res) => {
      if (res !== "NO")
        this.setState({
  ...
  componentDidMount() {
    // fetch(
    //   `http://127.0.0.1:3001/getInventory/`
    // )
    //   .then((res) => res.json())
    ...
    fetch(
      `http://127.0.0.1:3001/patient/getPatientMatrics/${patient.PATIENT_ID}`
    )
      .then((res) => res.json())
      .then((res) => {
        if (res !== "NO")
    ...
    fetch("http://127.0.0.1:3001/patient/getDoctor")
  ...
  getAppointments = (heading) => {
    ...
    if (this.state.appointments.length) {
      ...
      let patient = this.props.patient;
      fetch(
        `http://127.0.0.1:3001/patient/getAppointments/${patient.PATIENT_ID}`
      )
        .then((res) => res.json())
        .then((res) => {
  ...
  handleGetPrescription = (heading) => {
    if (this.state.prescriptions.length) {
      ...
      let patient = this.props.patient;
      fetch(
        `http://127.0.0.1:3001/patient/getPrescriptions/${patient.PATIENT_ID}`
      )
        .then((res) => res.json())
        .then((res) => {
          if (res === "NO" && this._isMounted) {
  ...
  getFeedbackDocList = (heading) => {
    if (this.state.feedBackDocList.length) {
      ...
      let patient = this.props.patient;
      fetch(
        `http://127.0.0.1:3001/patient/getFeedBackDocList/${patient.PATIENT_ID}`
      )
        .then((res) => res.json())
        .then((res) => {
          if (res === "NO" && this._isMounted) {
  ...
  handleDeleteFeedback = (doc_id, sl_no) => {
    ...
    fetch("http://127.0.0.1:3001/patient/deleteFeedback", {
      method: "DELETE",
      headers: {
  ...
  handleGiveFeedback = (i, d_id, r) => {
    ...
    fetch("http://127.0.0.1:3001/patient/giveFeedback", load)
      .then((res) => res.json())
      .then((res) => {
        if (res === "SUCCESSFUL") {
