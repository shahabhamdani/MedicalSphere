...
class AdminHome extends Component {
  ...
  componentDidMount() {
    ...
    fetch("https://gotyour693.com:3001/admin/getRegisterDocs")
  ...
  handleGetValidDocs = (heading) => {
    if (this.state.validDoc.length) {
      ...
      fetch("https://gotyour693.com:3001/admin/getValidDocs")
        .then((response) => response.json())
        .then((res) => {
  ...
  handleGetAllDocs = (heading) => {
    if (this.state.allDocs.length) {
      ...
      fetch("https://gotyour693.com:3001/admin/getAllDocs")
        .then((response) => response.json())
        .then((res) => {
          if (res === "NO" && this._isMounted) {
            this.setState({
  ...
  handleValid = (id, i) => {
    ...
    fetch(`https://gotyour693.com:3001/admin/validDoc/${id}`);
  ...
  handleInvalid = (id, i) => {
    console.log("Valid: ", id);
    fetch(`https://gotyour693.com:3001/admin/invalidDoc/${id}`, { method: "delete" });
    let DocList = [...this.state.DocList];
    DocList.splice(i, 1);
    this.setState({ DocList });
  };
