...
const handleGetPrescriptions = (req, res, db) => {
    // console.log(req.params.id)
    return db.select('history.*','prescription.MED_CODE', 'validdoctor.NAME')
        .from('history')
...
const handleGetPatientMetrics = (req, res, db) => {
    ...
    db.select(
      db.raw(`(SELECT COUNT(*) FROM appointments WHERE PATIENT_ID = '${patientId}') AS APPOINTMENTS`),
      db.raw(`(SELECT COUNT(*) FROM confirmedappointments WHERE PATIENT_ID = '${patientId}') AS CONFIRMED_APPOINTMENTS`),
      db.raw(`(SELECT COUNT(*) FROM history WHERE PATIENT_ID = '${patientId}') AS HISTORY`)
    )
    .then(result => {
      const payload = {
...
const handleGetPharmacy = (req, res, db) => {
    // console.log(req.params.id)
    return db.select('prescription.MEDICINE')
        .from('prescription')
