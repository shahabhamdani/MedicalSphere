...
const handleSearchDoc = (req, res, db) => {
  ...
  db.select("*")
    ...
    .andWhere("SPECIALIZATION", "like", `%${specialization}%`)
    .then((docs) => {
      res.json(docs);
    });
};
...
const handleAvailableTimeSlot = (req, res, db) => {
  ...
  db.select("TIME")
    ...
    .then((notAvailable) => {
      ...
      if (available.length) res.json(available);
      else res.json("NA");
    });
};
  ...
  return (
    db
      .select(
        ...
        "DATE",
        "TIME",
        "validdoctor.PHONE_NUMBER"
      )
      .from("validdoctor")
      .innerJoin(
        "confirmedappointments",
        "validdoctor.DOC_ID",
        "confirmedappointments.DOC_ID"
      )
...
const handleGetPrescriptions = (req, res, db) => {
  // console.log(req.params.id)
  return db
    .select("history.*", "prescription.MED_CODE", "validdoctor.NAME")
    .from("history")
    .leftJoin("prescription", "history.MED_CODE", "prescription.MED_CODE")
    .leftJoin("validdoctor", "history.DOC_ID", "validdoctor.DOC_ID")
    .where("history.PATIENT_ID", "=", req.params.id)
    .catch((err) => console.log(err))

    .then((prescriptions) => {
      // console.log(appointments)
      if (prescriptions.length) res.json(prescriptions);
      else res.json("NO");
    })
    .catch((err) => res.json(err));
};

const handleGetPatientMetrics = (req, res, db) => {
  const patientId = req.params.pat_id; // get patient ID from request params

  db.select(
    db.raw(
      `(SELECT COUNT(*) FROM appointments WHERE PATIENT_ID = '${patientId}') AS APPOINTMENTS`
    ),
    db.raw(
      `(SELECT COUNT(*) FROM confirmedappointments WHERE PATIENT_ID = '${patientId}') AS CONFIRMED_APPOINTMENTS`
    ),
    db.raw(
      `(SELECT COUNT(*) FROM history WHERE PATIENT_ID = '${patientId}') AS HISTORY`
    )
  )
    .then((result) => {
      const payload = {
...
const handleGetFeedback = (req, res, db) => {
  ...
  db.select("*")
    .from("feedback")
    .leftJoin("validdoctor", "feedback.DOC_ID", "=", "validdoctor.DOC_ID")
...
const handleDeleteFeedback = (req, res, db) => {
    const { doc_id, sl_no } = req.body;
    db('feedback')
        .where({ SL_NO: sl_no })
        .del()
        .catch(err => console.log(err))
...
const handleGetPharmacy = (req, res, db) => {
  // console.log(req.params.id)
  return db
    .select("prescription.MEDICINE")
    .from("prescription")
    .where("MED_CODE", "=", req.params.id)
    .catch((err) => console.log(err))

    .then((pharmacy) => {
      // console.log(appointments)
      if (pharmacy.length) res.json(pharmacy);
      else res.json("NO");
    })
    .catch((err) => res.json(err));
};
...
const handleGetFeedBackDocList = (req, res, db) => {
  ...
  db.select("history.DOC_ID", "NAME", "SPECIALIZATION")
    ...
    .then((feedbackList) => {
      if (feedbackList.length) res.json(feedbackList);
      else res.json("NO");
    });
};
...
module.exports = {
  ...
  handleGetPrescriptions: handleGetPrescriptions,
  handleGetPharmacy: handleGetPharmacy,
  handleGetPatientMetrics: handleGetPatientMetrics,
